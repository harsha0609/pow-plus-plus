...
def add_block():
    ...
    block = Block(
        ...
        block_data['miner'], 
        block_data['nonce'], 
        block_data['prevcheckhash']
    )
    
    # Attempt to add the block to the blockchain
    ...
    # Block successfully added, reset checkpoints and update last main block
    blockchain.checkpoints_mined = {
        'checkpoint1': [],
        'checkpoint2': [],
        'checkpoint3': [],
        'final': []
    }
    blockchain.last_main_block = block  # Update last main block
    print(f"Block added to chain: {block.__dict__}")
    
    # Start a new mining cycle immediately after a block is added
    start_new_cycle(block_data['miner'])
...
def announce_winner():
    ...
    winner_block = Block(
        winner_block['index'], 
        winner_block['block_timestamp'], 
        winner_block['transactions'], 
        winner_block['prev_hash'], 
        winner_block['miner'], 
        winner_block['nonce']
    )
...
def start_new_cycle(miner):
    """
    Start a new cycle immediately after a main block is mined.
    Only the miner who mined the block can initiate a new cycle.
    All mining processes on other miners are stopped before starting a new cycle.
    """
    # Stop all mining processes on other miners
    for peer in peers:
        if peer != miner:
            try:
                response = requests.get(f'{peer}/stop_mining')
                if response.status_code == 200:
                    print(f"Stopped mining on peer: {peer}")
                else:
                    print(f"Failed to stop mining on peer: {peer}")
            except requests.exceptions.RequestException as e:
                print(f"Error stopping mining on peer {peer}: {e}")

    # Start a new mining cycle
    try:
        response = requests.get(f'http://localhost:{port}/start_mining', params={'miner': miner})
        if response.status_code == 200:
            print(f"New mining cycle started for miner: {miner}")
        else:
            print(f"Failed to start new mining cycle for miner: {miner}")
    except requests.exceptions.RequestException as e:
        print(f"Error starting new mining cycle for miner {miner}: {e}")
